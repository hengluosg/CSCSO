    ...
    for i, covariate in enumerate(covariates):
        ...
        for t in range(T):
            ...
            for l, region_points in enumerate(regions):
                ...
                user_weights =  np.ones(num_users)
                user_weights_list.append(user_weights)
                user_noise = np.abs(np.random.normal(0, sigma, num_users))
...
def basis_functions(x):
    ...
    #return np.hstack([np.ones((x.shape[0], 1)),x-1.5,(x-1.5)**2]) #
...
@ray.remote
def run_replication_lr(n, T, L, eta_0, BS_positions, test_points, seed =3):
...
def krr_cross_validation(covariates_points, theta_estimates, lambda_vals=[0.00001,0.0001, 0.001, 0.01, 0.1, 1, 10], n_splits=5, length_scale=1.0):
    """Perform K-fold cross-validation for different lambda_param values."""
    
    lambda_vals = np.arange(0.1,3,0.2)
...
@ray.remote
def run_replication(n, T, L, eta_0, BS_positions, test_points, seed =3):
    set_global_seed(seed)
    covariates = grid_sample(L, lowbound_cov, upbound_cov, n)
    regions = assign_regions(BS_positions)
    theta_estimates = simulation_algorithm(covariates, BS_positions, regions, L, n, T, eta_0, sigma)
...
@ray.remote
def run_ks_replication(n, T, L, eta_0, BS_positions,  test_points,  h, seed =3):
...
@ray.remote
def run_knn_replication(n, T, L, eta_0, BS_positions,  test_points, k, seed =3):
    set_global_seed(seed)
    covariates = grid_sample(L, lowbound_cov, upbound_cov, n)
    regions = assign_regions(BS_positions)
    theta_estimates = simulation_algorithm(covariates, BS_positions, regions, L, n, T, eta_0, sigma)
...
def picture_plot_stations(base_stations_list):
    fig, axes = plt.subplots(1, 3, figsize=(15, 5))
    ...
    for idx, base_stations in enumerate(base_stations_list):
        ...
        for i, region_points in enumerate(regions):
            if len(region_points) > 0:
                ...
                sns.scatterplot(x="X", y="Y", data=df, ax=axes[idx], color=color, s=30)
...
if __name__ == '__main__':
    ...
    picture_plot_stations(base_stations_list)
    ...
    for covariate_dim in covariate_dim1:
        ...
        n = 1  # num_test_points
        T = 1000  # Number of iterations per covariate
        covariates = np.random.uniform(lowbound_cov, upbound_cov, size=(n, L))
        
        regions = assign_regions(base_stations)
        
        
        
        averaged_params = simulation_algorithm(covariates, base_stations, regions, L, n, T, eta_0, sigma)
        print("Averaged Parameters:", averaged_params.shape)
        
        test_points, true_theta_values = covariates , averaged_params
        ...
        for i, gamma in enumerate(total_budget):
            ...
            replication_results = ray.get([run_replication_lr.remote(n, T, L, eta_0, base_stations, test_points,seed = j)  for j in range(replication)])
        ...
        for i, gamma in enumerate(total_budget):
            ...
            replication_results = ray.get([run_replication.remote(n, T, L, eta_0, base_stations, test_points,seed = j)  for j in range(replication)])
        ...
        for i, gamma in enumerate(total_budget):
            ...
            # n = round(gamma ** ((covariate_dim + 1) / (covariate_dim + 2)))
            # T = round(gamma / n)
            ...
            # Call the remote function to parallelize replications
            replication_results = ray.get([run_ks_replication.remote(n, T, L, eta_0, base_stations,test_points,  h,seed = j)  for j in range(replication)])
        ...
        for i, gamma in enumerate(total_budget):
            ...
            replication_results = ray.get([ run_knn_replication.remote(n, T, L, eta_0, base_stations, test_points, k,seed = j)  for j in range(replication)])
        ...
        df.to_csv('e2dim{}.csv'.format(covariate_dim), index=False)
