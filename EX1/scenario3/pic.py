...
def picture_plot1(df, covariate_dim1):
    ...
    plt.tight_layout()
    plt.savefig('ex_1_highdimension.pdf', format='pdf')



total_budget = 2 ** np.arange(14,20)
...
for covariate_dim in covariate_dim1:
    ...
    if covariate_dim == 20:
        df = pd.read_csv('dim20.csv')
        
    elif covariate_dim == 50:
        df = pd.read_csv('dim50.csv')
    else:
        df = pd.read_csv('dim100.csv')
